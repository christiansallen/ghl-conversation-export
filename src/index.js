...
app.post("/api/export", async (req, res) => {
  const { contactId, locationId, contactName, contactEmail, contactPhone } = req.body;
...
async function runExport(job, contact) {
  try {
    // Fetch all messages
    const messages = await conversations.fetchContactHistory(
      job.locationId,
      job.contactId,
      (progress) => {
